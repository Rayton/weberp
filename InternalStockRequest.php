...
if (isset($_POST['Update'])) {
    ...
    if ($_POST['ReceiveTo'] == '') {
        ...
        prnMsg(_('You must select location to receive stock'), 'error');
        $InputError = 1;
    }
    if ($_POST['ReceiveFrom'] == '') {
        prnMsg(_('You must select a Location to request the items from'), 'error');
        $InputError = 1;
    }

    if ($_POST['ReceiveTo'] == $_POST['ReceiveFrom']) {
        prnMsg(_('You cannot receive in the same location'), 'error');
        $InputError = 1;
    }

    if ($InputError == 0) {
        $_SESSION['Request']->ReceiveTo = $_POST['ReceiveTo'];
        $_SESSION['Request']->ReceiveFrom = $_POST['ReceiveFrom'];
...
if (isset($_POST['Submit'])) {
   ...
   // DB_Txn_Begin($db);
    $InputError = 0;
    if ($_SESSION['Request']->ReceiveTo == '') {
        prnMsg(_('You must select location to receive items'), 'error');
        $InputError = 1;
    }
    if ($_SESSION['Request']->ReceiveFrom == '') {
        prnMsg(_('You must select a Location to request the items from'), 'error');
        $InputError = 1;
    }
    if ($InputError == 0) {
        ...
        $HeaderSQL = "INSERT INTO stockrequest (dispatchid,
										...
										VALUES(
											'" . $RequestNo . "',
											'" . $_SESSION['Request']->ReceiveFrom . "',
											'" . $_SESSION['Request']->ReceiveTo . "',
        ...
        foreach ($_SESSION['Request']->LineItems as $LineItems) {
           ...
           // echo 'stockrequestitems '. $LineSQL;
...
echo '<td><select name="ReceiveTo">';
echo '<option value="">' . _('Select a Location') . '</option>';
while ($myrow = DB_fetch_array($result)) {
    if (isset($_SESSION['Request']->ReceiveTo) and $_SESSION['Request']->ReceiveTo == $myrow['loccode']) {
...
$result = DB_query($sql, $db);
echo '<td><select name="ReceiveFrom">';
echo '<option value="">' . _('Select a Location') . '</option>';
while ($myrow = DB_fetch_array($result)) {
    if (isset($_SESSION['Request']->ReceiveFrom) and $_SESSION['Request']->ReceiveFrom == $myrow['loccode']) {
        echo '<option selected="True" value="' . $myrow['loccode'] . '">' . $myrow['loccode'] . ' - ' . htmlentities($myrow['locationname'], ENT_QUOTES, 'UTF-8') . '</option>';
    } else {
        echo '<option value="' . $myrow['loccode'] . '">' . $myrow['loccode'] . ' - ' . htmlentities($myrow['locationname'], ENT_QUOTES, 'UTF-8') . '</option>';
    }
...
if (!isset($_SESSION['Request']->ReceiveFrom)) {
    include('includes/footer.php');
    exit;
}
...
echo '</table><br />';
echo '<div class="centre"><button type="submit" name="Submit">' . _('Submit') . '</button></div><br /></form>';
...
if (isset($SearchResult)) {
    ...
    while ($myrow = DB_fetch_array($SearchResult)) {
        ...
        $QOHSQL = "SELECT sum(locstock.quantity) AS qoh
							   ...
							   loccode = '" . $_SESSION['Request']->ReceiveFrom . "'";
        ...
        $sql = "SELECT SUM(salesorderdetails.quantity-salesorderdetails.qtyinvoiced) AS dem
						 ...
						 WHERE salesorders.orderno = salesorderdetails.orderno AND
							salesorders.fromstkloc='" . $_SESSION['Request']->ReceiveFrom . "' AND
 							salesorderdetails.completed=0 AND
		 					salesorders.quotation=0 AND
				 			salesorderdetails.stkcode='" . $myrow['stockid'] . "'";
        $ErrMsg = _('The demand for this product from') . ' ' . $_SESSION['Request']->ReceiveFrom . ' ' . _('cannot be retrieved because');
